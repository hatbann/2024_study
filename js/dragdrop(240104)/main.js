...
emoji1.addEventListener("dragover", (e) => {
  e.preventDefault();
});

emoji1.addEventListener("drop", (e) => {
  e.preventDefault();
  const emoji = emoji2.innerText;
  emoji2.innerText = "";
  emoji1.appendChild(document.createTextNode(emoji));
});

emoji2.addEventListener("dragover", (e) => {
  e.preventDefault();
});

emoji2.addEventListener("drop", (e) => {
  e.preventDefault();
  const emoji = emoji1.innerText;
  emoji1.innerText = "";
  emoji2.appendChild(document.createTextNode(emoji));
});
